    ...
    def get_queryset(self):
        ...
        if query:
            queryset = queryset.filter(
                ...
                Q(abstract__icontains=query) | Q(palavras_chave__icontains=query) |
                Q(orientador__user__first_name__icontains=query) | Q(orientador__user__last_name__icontains=query) |
                Q(aluno__user__first_name__icontains=query) | Q(aluno__user__last_name__icontains=query)
            )
...
class MonografiaCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def test_func(self):
        ...
        return user.groups.filter(name='Alunos').exists() and hasattr(user, 'aluno_profile')
    
    def form_valid(self, form):
        if hasattr(self.request.user, 'aluno_profile') and Monografia.objects.filter(aluno=self.request.user.aluno_profile).exists():
